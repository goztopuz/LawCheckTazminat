...
{
    public class NetBrutSonViewModel : ViewModelBase
    {
        public NetBrutSonViewModel(List<NetBrutAylar> _aylar)
        {

            this.Liste = new ObservableCollection<NetBrutAylar>();

            int ii = 0;
            foreach(var t in _aylar)
            { ii = ii + 1;
                t.ay = ii;
            }
        ...
        private async void RaporAl()
        {
            ...
            textRange = paragraph.AppendText("Netten Brüt Hesaplama".ToUpper()) as WTextRange;
            ...
            tabloDonemler.Rows[12].Height = 32;


            MemoryStream stream = new MemoryStream();
            document.Save(stream, FormatType.Docx);
            var sayfa = new Views.PdfV.PdfView(stream, "NettenBrutRapor.docx", "docx");
            await Application.Current.MainPage.Navigation.PushModalAsync(sayfa);
            //Save the stream as a file in the device and invoke it for viewing
         //   var yoll = await Xamarin.Forms.DependencyService.Get<ISave>().SaveAndView2("NettenBrutRapor.docx", "application/msword", stream);
